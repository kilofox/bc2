<div class="cell auto-size padding20 bg-white" id="cell-content">
	<h1 class="text-light">用户角色 <span class="mif-drive-eta place-right"></span></h1>
			...
			<?php foreach( $nodes as $node ): ?>
				<tr>
					...
					<td class="align-center">
						<a href="<?php echo $baseUrl; ?>/users/admin/role/<?php echo $node->id; ?>/edit">编辑</a>
						<a href="<?php echo $baseUrl; ?>/users/admin/role/<?php echo $node->id; ?>/delete" data-del="<?php echo $node->id; ?>">删除</a>
					</td>
				</tr>
			<?php endforeach; ?>
		</tbody>
...
<script>
	$(function(){
		$('button[data-btn="create"]').click(function(){
			window.location.href = baseUrl + '/users/admin/role/create';
		});
		$('a[data-del]').click(function(){
			alert('您确定要删除 ID #' + $(this).attr('data-del') + '：' + $(this).parent().siblings().eq(1).text() + ' 这个角色吗？');
			$.ajax({
				url: baseUrl + '/users/admin/role/' + $(this).attr('data-del') + '/delete',
				type: 'delete',
				dataType: 'json',
				data: $('#edit').serialize(),
				success: function(r){
