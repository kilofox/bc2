...
//				}
		...
		_initBar: function(){
			...
			// 按顺序排列的所有菜单项
			that.allMenuEntries = $();
			// 可以但不需要成为 this.element 的菜单的共同父级。我们从中得到最大宽度
			that.menusParent = $();
			that.pullMenu = $();
			if ( menus.length > 0 && $(element).is(":not('.no-flexible')") ){
				...
				if ( that.flexVisibles.length > 0 )
				{
					...
					// 由于某些原因，下拉菜单的 :after :before 的 clearfix 不工作。
					// 如果不打破 float， position: absolute 不工作。
					// 应该另写一个纯 css 解决方案，之后删除此行。
					...
					// 如果用户调整或缩放窗口，我们要计算每一样东西
					$(window).resize(function(){
						$('[data-role=appbar]:not(.no-flexible)').each(function(){
							$(this).data('appbar').resize();
						});
					});
					// 因为字体（包括图标字体）往往是在页面加载以后加载，而此脚本已经过去了，我们需要在这些元素加载之后再次检查。
					// 因为没有办法只观察特定元素，我们在窗口上进行。
					$(window).load(function(){
						$('[data-role=appbar]:not(.no-flexible)').each(function(){
							$(this).data('appbar').resize();
						});
					});
					// 加载图片（或其它外部的东西）。图片通常也是异步加载，或延迟加载，或过一段时间后注入。
					// 图片可以改变应用条上元素的大小，所以我们必须重新检查。
	...
	$.widget('metro.input', {
		...
		_createInputFile: function(){
			...
			wrapper = $('<input type="text" class="input-file-wrapper" readonly="readonly" style="z-index: 1; cursor: default;">');
			button = element.children('.button');
