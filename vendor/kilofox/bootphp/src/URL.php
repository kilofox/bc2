...
class URL
{
        ...
        if (is_string($protocol)) {
            ...
            if ($host = parse_url($baseUrl, PHP_URL_HOST)) {
                ...
                if (!self::isTrustedHost($host)) {
                    throw new BootphpException('Untrusted host `' . $host . '`. If you trust `' . $host . '`, add it to the trusted hosts in the `url` config file.');
                }
            }
    ...
    public static function site($uri = '', $protocol = null)
    {
        ...
        $path = preg_replace('#^[-a-z\d+.]++://[^/]++/?#', '', trim($uri, '/'));
     ...
     *
     * Tests against given $trustedHosts
     ...
     */
    public static function isTrustedHost($host, array $trustedHosts = [])
    {
