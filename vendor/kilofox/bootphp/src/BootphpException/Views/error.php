...
<div id="bootphp_error">
            ...
            <?php foreach (Debug::trace($trace) as $i => $step): ?>
                <li>
                    <p>
                        ...
                        &raquo;
                        <?php echo $step['function']; ?>(<?php if ($step['args']): $args_id = $error_id . 'args' . $i; ?><a href="#<?php echo $args_id; ?>" onclick="return toggle('<?php echo $args_id; ?>')">arguments</a><?php endif ?>)
                    </p>
    ...
    <h2>
        <a href="#<?php echo $env_id = $error_id . 'environment'; ?>" onclick="return toggle('<?php echo $env_id; ?>')">Environment</a>
    </h2>
    <div id="<?php echo $env_id; ?>" class="content collapsed">
        <?php $included = get_included_files(); ?>
        <h3>
            <a href="#<?php echo $env_id = $error_id . 'environment_included'; ?>" onclick="return toggle('<?php echo $env_id; ?>')">Included files</a> (<?php echo count($included); ?>)
        </h3>
        ...
        <h3>
            <a href="#<?php echo $env_id = $error_id . 'environment_loaded'; ?>" onclick="return toggle('<?php echo $env_id; ?>')">Loaded extensions</a> (<?php echo count($included); ?>)
        </h3>
        <div id="<?php echo $env_id; ?>" class="collapsed">
            <table cellspacing="0">
                <?php foreach ($included as $file): ?>
            ...
            <div id="<?php echo $env_id; ?>" class="collapsed">
                <table cellspacing="0">
                    <?php foreach ($GLOBALS[$var] as $key => $value): ?>
                        <tr>
                            <td>
                                <code><?php echo htmlspecialchars((string) $key, ENT_QUOTES, 'UTF-8', true); ?></code>
                            </td>
                            <td>
